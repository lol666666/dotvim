...
let g:MCB_tagBufNum       = -1
let s:MCB_tagForceRefresh =  0
...
if !exists('loaded_taglist')
    ...
    command! -nargs=0 -bar TlistOpen call Tlist_Window_Open()
    ...
    command! -nargs=0 -bar TlistHighlightTag call Tlist_Window_Highlight_Tag(
...
" c++ language
let s:tlist_def_cpp_settings = 'c++;f:function'
"let s:tlist_def_cpp_settings = 'c++;n:namespace;v:variable;d:macro;t:typedef;' .
"                             \ 'c:class;g:enum;s:struct;u:union;f:function'

" go language
let s:tlist_def_go_settings = 'go;f:function'
...
" perl language
let s:tlist_def_perl_settings = 'perl;s:subroutine'
...
" vim language
let s:tlist_def_vim_settings = 'vim;a:autocmds;f:function'
...
function! s:Tlist_Window_Display_Help()
    ...
    if s:tlist_brief_help
        ...
        call append(10, '" = : Close all folds')
        call append(11, '" [[ : Move to the start of previous file')
        call append(12, '" ]] : Move to the start of next file')
        call append(13, '" q : Close the taglist window')
        call append(14, '" ? : Remove help text')
    endif
endfunction
...
function! s:Tlist_Window_Create()
    ...
    if bufnum == -1
        " Create a new buffer
        enew
        let g:MCB_tagBufNum = bufnr('%')
        silent execute 'file ' . g:TagList_title
    else
        " Edit the existing buffer
        silent execute "buffer " . bufnum
        let g:MCB_tagBufNum = bufnum
    endif
...
function! s:Tlist_Window_Init()
    ...
    silent! setlocal statusline=%f%<%=[L=%02l]\ [%P]
    ...
    augroup TagListAutoCmds
        ...
        autocmd CursorHold * silent call Tlist_Window_Highlight_Tag(
...
function! Tlist_Window_Open()
    ...
    " If the window is open, make sure it's up-to-date.
    if ( bufexists(g:MCB_tagBufNum) && ( getbufvar( g:MCB_tagBufNum, '&filetype' ) == 'taglist' ) )
        let curbuf_name  = fnamemodify(bufname('%'), ':p')
        let curbuf_ftype = s:Tlist_Get_Buffer_Filetype('%')
        silent execute g:MCB_sidebarWinNum . 'wincmd w' 

        if ( ( s:tlist_cur_file_idx == -1 ) || ( s:tlist_{s:tlist_cur_file_idx}_filename != curbuf_name ) || s:MCB_tagForceRefresh )
            let fidx = s:Tlist_Get_File_Index(curbuf_name)
            if ( fidx != -1 )
                let s:tlist_{fidx}_valid = 0
            endif

            if ( s:MCB_tagForceRefresh )
                call Tlist_Window_Refresh()
                let s:MCB_tagForceRefresh = 0
            endif
    ...
    call Tlist_Window_Highlight_Tag(curbuf_name, cur_lnum, 1, 1)
endfunction
...
function! Tlist_Refresh()
    ...
    if tlist_win != -1
        ...
        if g:Tlist_Highlight_Tag_On_BufEnter && g:Tlist_Auto_Highlight_Tag
            ...
            " Highlight the current tag
            call Tlist_Window_Highlight_Tag(filename, cur_lnum, 1, center_tag_line)
        else
            " Restore the cursor position
            if v:version >= 601
                call cursor(save_line, save_col)
...
function! Tlist_Window_Highlight_Tag(filename, cur_lnum, cntx, center)

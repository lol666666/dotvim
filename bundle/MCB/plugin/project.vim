...
function! s:Project(filename)
    ...
    function! s:OpenEntry2(line, infoline, fname, editcmd)
        ...
        if a:editcmd[0] != '' " If editcmd is '', then just set up the environment in the Project Window
            call s:DoSetupAndFocusOnEditWin()
    ...
    if !exists("g:proj_running")
        ...
        exec 'au BufEnter '.bufname.' call s:DoSetup_au()'
        exec 'au BufEnter '.bufname.' nested call s:ExitOnlyWindow()'
        " >>>
        setlocal buflisted
        let g:proj_running = bufnr(bufname.'\>')
        let g:MCB_projBufNum = g:proj_running
        if g:proj_running == -1
